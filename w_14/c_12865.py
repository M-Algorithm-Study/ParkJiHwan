...
for i in range(1, n + 1):
    for j in range(1, k + 1): # 가방의 무게를 j로 제한하면,( k = 7 이라면 j는 1 ~ 7)
        ...
        if j < w: # 제한된 무게(j) < 물건의 무게(w) 일 때: 이전 값을 그대로 가져옴
            dp[i][j] = dp[i-1][j]
        else: # 물건의 무게(w) =< 제한된 무게(j) : max(이전 값을 그대로 가져옴, 이전 j - w의 값에 v를 더한 값을 가져옴)
...
# dp:
# [[0, 0, 0, 0, 0, 0, 0, 0],    -> i = 0
#  [0, 0, 0, 0, 0, 0, 13, 13],  -> i = 1
#  [0, 0, 0, 0, 8, 8, 13, 13],  -> i = 2
#  [0, 0, 0, 6, 8, 8, 13, 14],  -> i = 3
#  [0, 0, 0, 6, 8, 12, 13, 14]] -> i = 4